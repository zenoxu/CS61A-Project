...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    Selectable_paragraphgs = [paragraph for paragraph in paragraphs if select(paragraph)]
    if k >= len(Selectable_paragraphgs):
        return ''
    else:
        return Selectable_paragraphgs[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def result_function(paragraph):
        paragraph_modify = split(lower(remove_punctuation(paragraph)))
        for word in topic:
            if word in paragraph_modify:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    diff_dic = {word: diff_function(typed_word, word, limit) for word in word_list}
    lowest_diff_word = min(diff_dic, key=lambda x: diff_dic[x])
    if diff_dic[lowest_diff_word] > limit:
        return typed_word
    else:
...
def sphinx_swaps(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def track_counts_sphni_swaps(start, goal, limit, count):
        if count > limit:
            return count
        ...
        if start[0] == goal[0]:
            # //print("DEBUG:", len(start))
            return track_counts_sphni_swaps(start[1:], goal[1:], limit, count)
        else:
            # print("DEBUG:", len(start))
            return track_counts_sphni_swaps(start[1:], goal[1:], limit, count + 1) + 1
    return track_counts_sphni_swaps(start, goal, limit, count=0)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    3
    """
    def track_counts_minimum_mewtations(start, goal, limit, counts):
        if counts > limit:
            return counts
        if len(start) == 0 or len(goal) == 0:  # Fill in the condition
            ...
            # BEGIN
            return track_counts_minimum_mewtations(start[1:], goal[1:], limit, counts)
            # END

        else:
            add = track_counts_minimum_mewtations(start, goal[1:], limit, counts + 1)
            remove = track_counts_minimum_mewtations(start[1:], goal, limit, counts + 1)  # remove start[0]
            substitute = track_counts_minimum_mewtations(start[1:], goal[1:], limit, counts + 1)
            # BEGIN
            return min(add, remove, substitute) + 1
            # END
    return track_counts_minimum_mewtations(start, goal, limit, counts=0)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    dict_for_upload = {'id': user_id, 'progress': progress}
    upload(dict_for_upload)
    return progress
    # END PROBLEM 8
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    result = []
    for i in range(0, len(match["times"])):
        result.append([])
