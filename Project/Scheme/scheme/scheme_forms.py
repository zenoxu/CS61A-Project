...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        while check_pointer is not nil:
            if not (scheme_symbolp(check_pointer.first)):
                raise SchemeError('non-symbol: {0}'.format(check_pointer.first))
            check_pointer = check_pointer.rest
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        if is_scheme_false(value):
            return False
        elif expressions.rest.rest == nil:
            return scheme_eval(expressions.rest.first, env, True)
        else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        val = scheme_eval(current.rest.first, env)
